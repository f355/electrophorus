...
static inline uint32_t rpi4_rd32(volatile uint8_t *base, uint32_t off) {
...
int rpi4_spi_init(int frequency_hz) {
  ...
  rpi4_bcm_bar = (volatile uint8_t *)mmap(NULL, BCM_PERI_MAP_LEN, PROT_READ | PROT_WRITE,
                                     MAP_SHARED, rpi4_mem_fd, (off_t)BCM2711_PERI_BASE);
  if (rpi4_bcm_bar == MAP_FAILED) {
    int e = -errno;
    close(rpi4_mem_fd); rpi4_mem_fd = -1;
    return e;
  }
