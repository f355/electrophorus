...
int rtapi_app_main(void) {
  ...
  for (int i = 0; i < STEPGENS; i++) {
    ...
    state->stepgens[i].prev_position = 0.0f;
  }
...
static void send_config() {
  ...
  for (int i = 0; i < STEPGENS; i++) {
    ...
    tx_data.stepper_position_scale[i] = (float)fabs(s->hal.param.position_scale);
    tx_data.stepper_max_accel[i] = (float)s->hal.param.maxaccel;
    tx_data.stepper_init_position[i] = (float)*s->hal.pin.position_cmd;
  }
        ...
        case PRU_DATA:
          ...
          *state->spi_status = 1;
          ...
          for (int i = 0; i < STEPGENS; i++) {
            ...
            *s->hal.pin.position_fb = (double)rx_data.stepgen_feedback[i];
...
void spi_write() {
  ...
  for (int i = 0; i < STEPGENS; i++) {
    ...
    tx_data.stepgen_freq_command[i] = (float)*s->hal.pin.position_cmd;
