...
vector<Module*> stepper_modules(const SpiComms* comms) {
  // XYZA
  return {new Stepgen(0, new Pin(1, 28), (new Pin(1, 29))->invert(), comms->rx_data, comms->tx_data),
          new Stepgen(1, new Pin(1, 26), new Pin(1, 27), comms->rx_data, comms->tx_data),
          new Stepgen(2, new Pin(1, 24), (new Pin(1, 25))->invert(), comms->rx_data, comms->tx_data),
          new Stepgen(3, new Pin(1, 21), (new Pin(1, 23))->invert(), comms->rx_data, comms->tx_data)};
}
