...
SpiComms::SpiComms() {
  ...
  const auto& spi = MachineSpiConfig();
  ...
  tx_lli_.dst = reinterpret_cast<uint32_t>(&spi.ssp->DR);
  tx_lli_.next = reinterpret_cast<uint32_t>(&tx_lli_);
  tx_lli_.ctrl = CtrlWord(true, false);

  rx_lli_.src = reinterpret_cast<uint32_t>(&spi.ssp->DR);
...
void SpiComms::Start() {
  const auto& spi = MachineSpiConfig();
  new SPISlave(spi.mosi, spi.miso, spi.sck, spi.ssel);
  ...
  tx->DMACCConfig = ConfigWord(kTransferM2p, spi.dma_conn_tx);
  ...
  rx->DMACCConfig = ConfigWord(kTransferP2m, spi.dma_conn_rx);

  LPC_GPDMA->DMACConfig = 1;
  tx->DMACCConfig |= 1;
  rx->DMACCConfig |= 1;

  spi.ssp->DMACR = (1 << 1) | (1 << 0);
}
